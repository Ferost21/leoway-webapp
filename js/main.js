if (typeof window.Telegram === 'undefined' || !window.Telegram.WebApp) {
    ...
    window.Telegram.WebApp = {
        initDataUnsafe: {
            user: {
                id: 674387089, // Заміни на свій реальний Telegram ID (отримай через @userinfobot)
                first_name: "Олексій",
                photo_url: "https://t.me/i/userpic/320/G5zXCocuPu5ixPhQRlIfJYmWJwGDWhMbv4Ou2ZRUFOM.svg"
            }
        },
        ready: () => {
            console.log("Mock WebApp ready"); // Додано для дебагу
        },
        onEvent: (eventType, callback) => {
            if (eventType === 'themeChanged') callback();
        },
        themeParams: {
            bg_color: '#f5f5f5',
            text_color: '#222',
        ...
        // === ДОДАЙТЕ ЦІ РЯДКИ ===
        setHeaderColor: (color, type = 'bg_color') => {
            console.log(`Mock setHeaderColor called with color: ${color}, type: ${type}`);
            // Тут можна додати логіку для зміни кольору, якщо потрібно для мокапу,
            // наприклад, змінювати CSS-змінну або просто логувати.
        },
        setBackgroundColor: (color) => {
            console.log(`Mock setBackgroundColor called with color: ${color}`);
        }
        // ========================
    };
}
