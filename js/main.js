...
function updateTheme() {
    ...
    root.style.setProperty('--button-bg-color', themeParams.button_color || (isDark ? '#2ea6ff' : '#3390ec'));
    root.style.setProperty('--button-text-color', themeParams.button_text_color || '#ffffff');
    root.style.setProperty('--hint-color', themeParams.hint_color || (isDark ? '#aaa' : '#666'));
    root.style.setProperty('--link-color', themeParams.link_color || (isDark ? '#6ab2f2' : '#2481cc'));
    root.style.setProperty('--secondary-bg-color', themeParams.secondary_bg_color || (isDark ? '#232323' : '#f5f5f5'));

    // Фон для input
    root.style.setProperty('--input-bg-color', themeParams.section_bg_color || (isDark ? '#2c3839' : '#ffffff'));

    // Колір тексту input завжди контрастний
    const inputTextColor = themeParams.text_color || (isDark ? '#ffffff' : '#000000');
    root.style.setProperty('--input-text-color', inputTextColor);
}
